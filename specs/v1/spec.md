...
`pattern` is a JSON Schema regular expression, so it is written and matched against the ECMA-262 regex dialect. A digit class (`\d`) in a `pattern` MUST be read as ASCII `[0-9]`, not the wider set of Unicode decimal digits: ECMA-262's `\d` is already ASCII-only, but some validator implementations run patterns through a host regex engine whose `\d` matches Unicode digits by default, and the ASCII reading is what independent validators need to converge on for identical codes on identical documents. The one v1 schema pattern with a digit class, `$defs/timestamp`, always pairs `pattern` with `format: date-time`, and the format check independently rejects non-ASCII digits — so the ASCII reading is latent in v1 today. It matters once a future pattern uses a digit class without an equally strict `format` alongside it.
